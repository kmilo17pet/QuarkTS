                       ...
                       void *ptr )
{
    ...
    if ( NULL != mPool ) {
        ...
        if ( NULL != ptr ) {
            ...
            /* memory being freed will have an qMemBlockConnect_t immediately before it. */    
...
static void qMemMang_HeapInit( qMemMang_Pool_t *mPool )
{
    ...
    /* Start is used to hold a pointer to the first item in the list of free blocks*/
        ...
        if ( (size_t)0u == ( pSize & blockAllocatedBit ) ) {
            if ( pSize > (size_t)0 ) { 
                ...
                */
                ...
                /*Ensure blocks are always aligned to the required number of bytes. */
