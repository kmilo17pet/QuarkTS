                                 ...
                                 const qTask_NotifyMode_t mode )
{
    qBool_t retValue = qFalse;
        ...
        if ( NULL ==  kernel.NotificationSpreadRequest.mode ) { 
            ...
            _QTRACE_KERNEL( "(E)Another notification spread operation is in progress. Request ignored", NULL, NULL );
        }
  
    #else
        Q_UNUSED( eventdata );
        Q_UNUSED( mode );    
        _QTRACE_KERNEL( "(E)Notification spread its disabled. Request ignored", NULL, NULL );
...
static void qOS_PriorityQueue_ClearIndex( qIndex_t indexToClear )
{
    ...
    for ( j = indexToClear ; (qBase_t)j < queueIndex ; ++j ) { 
        kernel.QueueStack[ j ] = kernel.QueueStack[ j + (qIndex_t)1u ]; /*shift the remaining items of the queue*/
    }
                                  ...
                                  void *pData )
{
        ...
        if ( ( NULL != Task )  && ( currentQueueIndex < queueMaxIndex ) ) {/*check if data can be queued*/
            ...
            /*cstat +CERT-INT32-C_a*/
            retValue = qTrue;
            _QTRACE_KERNEL( "(>)Event inserted on priority queue for task ", Task, pData );
        }
        else {
            _QTRACE_KERNEL( "(E)Event not inserted on priority queue. Queue full or invalid data.", NULL, NULL );
        }
        return retValue;
    #else
        Q_UNUSED( Task );
        Q_UNUSED( Data );
        _QTRACE_KERNEL( "(E)Event not inserted inserted on priority queue. Queue its disabled", NULL, NULL );
        return qFalse;
    #endif   
}
/*========================== Shared Private Method ===========================*/
                      ...
                      void* arg )
{
    ...
    if ( NULL != Task ) {
        ...
        Task->qPrivate.Priority = ( p > ( (qPriority_t)Q_PRIORITY_LEVELS - (qPriority_t)1u ) )? 
        ...
        _QTRACE_KERNEL( "(*)Insertion Performed to waiting-list for task ", Task, NULL ); 
    }
    else {
                                  ...
                                  void *arg )
{
    ...
    if ( NULL != m ) {
        if ( qTrue == qOS_Add_Task( Task, qOS_FSM_TaskCallback, p, t, qPeriodic, init, arg ) ) {
            ...
            #endif
            _QTRACE_KERNEL( "(>)State-machine successfully attached to task ", Task, m );
        }
    }
...
static void qOS_ATCLI_NotifyFcn( qATCLI_t * const cli )
{
    ...
    /*cstat +MISRAC2012-Rule-11.5 +CERT-EXP36-C_b*/
    (void)qTask_Notification_Send( xTask, NULL ); /*simple notifications preferred because queued notification can be disabled on <qconfig.h> */
}
#endif /* #if ( Q_ATCLI == 1 ) */
/*============================================================================*/
...
static qTrigger_t qOS_AttachedQueue_CheckEvents( const qTask_t * const Task )
{
    ...
    if ( NULL != Task->qPrivate.Queue ) {
        ...
        if ( fullFlag && qQueue_IsFull( Task->qPrivate.Queue ) ) { /*qQueue_IsFull is known to have no side effects*/   
            _QTRACE_KERNEL( "(>)Event::IsFull on attached-queue for task ", Task, Task->qPrivate.Queue );  
            retValue =  byQueueFull;         
        }
        else if ( ( countFlag ) && ( currentQueueCount >= Task->qPrivate.QueueCount ) ) {
            _QTRACE_KERNEL( "(>)Event::Count on attached-queue for task ", Task, Task->qPrivate.Queue );            
            retValue =  byQueueCount;
        }
        else if ( receiverFlag && ( currentQueueCount > 0u ) ) {    
            _QTRACE_KERNEL( "(>)Event::Receiver on attached-queue for task ", Task, Task->qPrivate.Queue );
            ...
            _QTRACE_KERNEL( "(>)Event::IsEmpty on attached-queue for task ", Task, Task->qPrivate.Queue );       
...
qBool_t qOS_Run( void )
{
    ...
    do {           
        if ( qList_ForEach( WaitingList, qOS_CheckIfReady, NULL, QLIST_FORWARD, NULL ) ) { /*check for ready tasks in the waiting-list*/
            qIndex_t xPriorityListIndex = (qIndex_t)Q_PRIORITY_LEVELS - (qIndex_t)1u;
    ...
    if ( qList_WalkThrough == h->stage ) {
        ...
        if ( qOS_Get_TaskFlag( xTask, QTASK_BIT_SHUTDOWN ) ) {
            ...
            if ( byNotificationQueued == xTask->qPrivate.Trigger ) {
                _QTRACE_KERNEL( "(<)Event {byNotificationQueued} detected on task ", xTask, NULL );
                xReady = qTrue;
            }
            else
            #endif 
            if ( qOS_TaskDeadLineReached( xTask ) ) { /*nested check for timed task, check the first requirement(the task must be enabled)*/
                _QTRACE_KERNEL( "(<)Event {byTimeElapsed} detected on task ", xTask, NULL );
                ...
                _QTRACE_KERNEL( "(<)Event {AttachedQueue} detected on task ", xTask, NULL );
                ...
                _QTRACE_KERNEL( "(<)Event {byEventFlags} detected on task ", xTask, NULL );
...
static qTrigger_t qOS_Dispatch_xTask_FillEventInfo( qTask_t *Task )
{
    ...
    switch ( xEvent ) { /*take the necessary actions before dispatching, depending on the event that triggered the task*/
        case byTimeElapsed:
        ...
        if ( NULL != xList ) { /*#!OK* false-positive can be reported here*/     
            ...
            if ( NULL != taskActivities ) {
                ...
                while ( NULL != kernel.YieldTask ) {
                    ...
                    kernel.YieldTask = NULL;
                    if ( NULL != taskActivities ) {
                        _QTRACE_KERNEL( "(^)Control yielded to task ", kernel.CurrentRunningTask, NULL );
                        taskActivities( &kernel.EventInfo ); /*yielded task inherits eventdata*/
                    }
                } 
            #endif
            ...
            (void)qList_Insert( WaitingList, xTask, QLIST_ATBACK );  /*and insert the task back to the waiting-list*/
            #if ( Q_QUEUES == 1 ) 
                if ( byQueueReceiver == xEvent ) {
                    (void)qQueue_RemoveFront( xTask->qPrivate.Queue );  /*remove the data from the Queue, if the event was byQueueDequeue*/
                } 
            #endif
            
            qOS_Set_TaskFlags( xTask, QTASK_BIT_INIT, qTrue ); /*set the init flag*/
